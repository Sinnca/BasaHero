//package com.basahero.elearning.worker
//
//import android.content.Context
//import android.util.Log
//import androidx.work.*
//import com.basahero.elearning.data.local.AppDatabase
//import com.basahero.elearning.data.local.DatabaseSeeder
//import com.basahero.elearning.data.remote.SupabaseClient
//import io.github.jan.supabase.postgrest.from
//import kotlinx.coroutines.Dispatchers
//import kotlinx.coroutines.withContext
//import kotlinx.serialization.Serializable
//import java.util.concurrent.TimeUnit
//import kotlin.random.Random
//
//// ─────────────────────────────────────────────────────────────────────────────
//// SyncProgressWorker
//// Uploads all unsynced STUDENT_PROGRESS rows to Supabase.
//// ─────────────────────────────────────────────────────────────────────────────
//class SyncProgressWorker(
//    context: Context,
//    params: WorkerParameters
//) : CoroutineWorker(context, params) {
//
//    private val tag = "SyncProgressWorker"
//    private val db = AppDatabase.getInstance(context, DatabaseSeeder(context))
//
//    companion object {
//        const val WORK_NAME = "sync_progress"
//
//        @Serializable
//        data class ProgressRow(
//            val id: String,
//            val student_id: String,
//            val lesson_id: String,
//            val status: String,
//            val quiz_score: Int,
//            val quiz_total: Int,
////            val completed_at: Long?,
////            val synced_at: Long = System.currentTimeMillis()
//            val completed_at: String?, // 👈 Change to String?
//            val synced_at: String      // 👈 Change to String
//        )
//
//        fun enqueue(context: Context) {
//            val constraints = Constraints.Builder()
//                .setRequiredNetworkType(NetworkType.CONNECTED)
//                .build()
//
//            val jitterSeconds = Random.nextLong(0L, 30L)
//
//            val request = OneTimeWorkRequestBuilder<SyncProgressWorker>()
//                .setConstraints(constraints)
//                .setInitialDelay(jitterSeconds, TimeUnit.SECONDS)
//                .setBackoffCriteria(
//                    BackoffPolicy.EXPONENTIAL,
//                    WorkRequest.MIN_BACKOFF_MILLIS,
//                    TimeUnit.MILLISECONDS
//                )
//                .addTag(WORK_NAME)
//                .build()
//
//            WorkManager.getInstance(context).enqueueUniqueWork(
//                WORK_NAME,
//                ExistingWorkPolicy.KEEP,
//                request
//            )
//        }
//    }
//
//    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
//        Log.d(tag, "Starting progress sync...")
//
//        return@withContext try {
//            val unsyncedRows = db.progressDao().getUnsynced()
//
//            if (unsyncedRows.isEmpty()) {
//                Log.d(tag, "Nothing to sync — all progress already uploaded.")
//                return@withContext Result.success()
//            }
//
//            Log.d(tag, "Uploading ${unsyncedRows.size} progress records...")
//
//            val chunks = unsyncedRows.chunked(50)
//            var successCount = 0
//            var failCount = 0
//
//            chunks.forEach { chunk ->
//                try {
//                    val rows = chunk.map { entity ->
//                        ProgressRow(
//                            id = entity.id,
//                            student_id = entity.studentId,
//                            lesson_id = entity.lessonId,
//                            status = entity.status.toString(), // FIXED: .name removed, using .toString() for safety
//                            quiz_score = entity.quizScore,
//                            quiz_total = entity.quizTotal,
////                            completed_at = entity.completedAt,
////                            synced_at = System.currentTimeMillis()
//                            completed_at = entity.completedAt?.let { java.time.Instant.ofEpochMilli(it).toString() },
//                            synced_at = java.time.Instant.now().toString()
//                        )
//                    }
//
//                    SupabaseClient.client
//                        .from("student_progress")
//                        .upsert(rows) {
//                            onConflict = "id"
//                        }
//
//                    // Uses your new highly efficient batch update DAO method!
//                    val chunkIds = chunk.map { it.id }
//                    db.progressDao().markSyncedBatch(chunkIds)
//
//                    successCount += chunk.size
//                    Log.d(tag, "✓ Chunk uploaded: ${chunk.size} records")
//
//                } catch (e: Exception) {
//                    failCount += chunk.size
//                    Log.e(tag, "✗ Chunk upload failed: ${e.message}")
//                }
//            }
//
//            if (failCount > 0 && successCount == 0) {
//                Result.retry()
//            } else {
//                Result.success()
//            }
//
//        } catch (e: Exception) {
//            Log.e(tag, "Sync failed entirely: ${e.message}")
//            Result.retry()
//        }
//    }
//}
//
//// ─────────────────────────────────────────────────────────────────────────────
//// SyncPronunciationWorker
//// Uploads all unsynced PRONUNCIATION_ATTEMPT rows to Supabase.
//// ─────────────────────────────────────────────────────────────────────────────
//class SyncPronunciationWorker(
//    context: Context,
//    params: WorkerParameters
//) : CoroutineWorker(context, params) {
//
//    private val tag = "SyncPronunciationWorker"
//    private val db = AppDatabase.getInstance(context, DatabaseSeeder(context))
//
//    companion object {
//        const val WORK_NAME = "sync_pronunciation"
//
//        @Serializable
//        data class PronunciationRow(
//            val id: String,
//            val student_id: String,
//            val lesson_id: String,
//            val attempt_number: Int,
//            val score: Int,
//            val is_best: Boolean,
//            val transcript: String,
//            val feedback_json: String,
//            val attempted_at: Long,
//            val synced_at: Long = System.currentTimeMillis()
//        )
//
//        fun enqueue(context: Context) {
//            val constraints = Constraints.Builder()
//                .setRequiredNetworkType(NetworkType.CONNECTED)
//                .build()
//
//            val jitterSeconds = Random.nextLong(5L, 35L)
//
//            val request = OneTimeWorkRequestBuilder<SyncPronunciationWorker>()
//                .setConstraints(constraints)
//                .setInitialDelay(jitterSeconds, TimeUnit.SECONDS)
//                .setBackoffCriteria(
//                    BackoffPolicy.EXPONENTIAL,
//                    WorkRequest.MIN_BACKOFF_MILLIS,
//                    TimeUnit.MILLISECONDS
//                )
//                .addTag(WORK_NAME)
//                .build()
//
//            WorkManager.getInstance(context).enqueueUniqueWork(
//                WORK_NAME,
//                ExistingWorkPolicy.KEEP,
//                request
//            )
//        }
//    }
//
//    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
//        Log.d(tag, "Starting pronunciation sync...")
//
//        return@withContext try {
//            val unsyncedAttempts = db.pronunciationDao().getUnsynced()
//
//            if (unsyncedAttempts.isEmpty()) {
//                return@withContext Result.success()
//            }
//
//            val chunks = unsyncedAttempts.chunked(50)
//
//            chunks.forEach { chunk ->
//                try {
//                    val rows = chunk.map { entity ->
//                        PronunciationRow(
//                            id = entity.id,
//                            student_id = entity.studentId,
//                            lesson_id = entity.lessonId,
//                            attempt_number = entity.attemptNumber,
//                            score = entity.score,
//                            is_best = entity.isBest,
//                            transcript = entity.transcript,
//                            feedback_json = entity.feedbackJson,
//                            attempted_at = entity.attemptedAt,
//                            synced_at = System.currentTimeMillis()
//                        )
//                    }
//
//                    SupabaseClient.client
//                        .from("pronunciation_attempt")
//                        .upsert(rows) {
//                            onConflict = "id"
//                        }
//
//                    val chunkIds = chunk.map { it.id }
//                    db.pronunciationDao().markSyncedBatch(chunkIds)
//
//                } catch (e: Exception) {
//                    Log.e(tag, "✗ Pronunciation chunk failed: ${e.message}")
//                }
//            }
//            Result.success()
//
//        } catch (e: Exception) {
//            Result.retry()
//        }
//    }
//}
//
//// ─────────────────────────────────────────────────────────────────────────────
//// SyncStudentWorker
//// Pulls NEW students added by teacher from Supabase down to Room.
//// ─────────────────────────────────────────────────────────────────────────────
//class SyncStudentWorker(
//    context: Context,
//    params: WorkerParameters
//) : CoroutineWorker(context, params) {
//
//    private val tag = "SyncStudentWorker"
//    private val db = AppDatabase.getInstance(context, DatabaseSeeder(context))
//
//    companion object {
//        const val WORK_NAME = "sync_students"
//
//        fun enqueue(context: Context) {
//            val constraints = Constraints.Builder()
//                .setRequiredNetworkType(NetworkType.CONNECTED)
//                .build()
//
//            val jitterSeconds = Random.nextLong(0L, 15L)
//
//            val request = OneTimeWorkRequestBuilder<SyncStudentWorker>()
//                .setConstraints(constraints)
//                .setInitialDelay(jitterSeconds, TimeUnit.SECONDS)
//                .setBackoffCriteria(
//                    BackoffPolicy.EXPONENTIAL,
//                    WorkRequest.MIN_BACKOFF_MILLIS,
//                    TimeUnit.MILLISECONDS
//                )
//                .addTag(WORK_NAME)
//                .build()
//
//            WorkManager.getInstance(context).enqueueUniqueWork(
//                WORK_NAME,
//                ExistingWorkPolicy.REPLACE,
//                request
//            )
//        }
//    }
//
//    @Serializable
//    data class StudentRow(
//        val id: String,
//        val class_id: String,
//        val full_name: String,
//        val section: String,
//        val grade_level: Int,
//        val last_active: Long?,
//        val created_at: Long
//    )
//
//    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
//        Log.d(tag, "Syncing student roster from Supabase...")
//
//        return@withContext try {
//            val remoteStudents = SupabaseClient.client
//                .from("student")
//                .select()
//                .decodeList<StudentRow>()
//
//            remoteStudents.forEach { row ->
//                val entity = com.basahero.elearning.data.local.entity.StudentEntity(
//                    id = row.id,
//                    classId = row.class_id,
//                    fullName = row.full_name,
//                    section = row.section,
//                    gradeLevel = row.grade_level,
//                    lastActive = row.last_active,
//                    synced = true,
//                    createdAt = row.created_at
//                )
//                // Uses the exact insertOrUpdate method from your updated StudentDao
//                db.studentDao().insertOrUpdate(entity)
//            }
//
//            Log.d(tag, "✓ Student roster sync complete")
//            Result.success()
//
//        } catch (e: Exception) {
//            Log.e(tag, "Student sync failed: ${e.message}")
//            Result.retry()
//        }
//    }
//}
package com.basahero.elearning.worker

import android.content.Context
import android.util.Log
import androidx.work.*
import com.basahero.elearning.data.local.AppDatabase
import com.basahero.elearning.data.local.DatabaseSeeder
import com.basahero.elearning.data.remote.SupabaseClient
import io.github.jan.supabase.postgrest.from
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import kotlinx.serialization.Serializable
import java.util.concurrent.TimeUnit
import kotlin.random.Random

// ─────────────────────────────────────────────────────────────────────────────
// SyncProgressWorker
// Uploads all unsynced STUDENT_PROGRESS rows to Supabase.
// ─────────────────────────────────────────────────────────────────────────────
class SyncProgressWorker(
    context: Context,
    params: WorkerParameters
) : CoroutineWorker(context, params) {

    private val tag = "SyncProgressWorker"
    private val db = AppDatabase.getInstance(context, DatabaseSeeder(context))

    companion object {
        const val WORK_NAME = "sync_progress"

        @Serializable
        data class ProgressRow(
            val id: String,
            val student_id: String,
            val lesson_id: String,
            val status: String,
            val quiz_score: Int,
            val quiz_total: Int,
            val completed_at: String?, // Supabase expects an ISO-8601 String
            val synced_at: String
        )

        fun enqueue(context: Context) {
            val constraints = Constraints.Builder()
                .setRequiredNetworkType(NetworkType.CONNECTED)
                .build()

            val jitterSeconds = Random.nextLong(0L, 30L)

            val request = OneTimeWorkRequestBuilder<SyncProgressWorker>()
                .setConstraints(constraints)
                .setInitialDelay(jitterSeconds, TimeUnit.SECONDS)
                .setBackoffCriteria(
                    BackoffPolicy.EXPONENTIAL,
                    WorkRequest.MIN_BACKOFF_MILLIS,
                    TimeUnit.MILLISECONDS
                )
                .addTag(WORK_NAME)
                .build()

            WorkManager.getInstance(context).enqueueUniqueWork(
                WORK_NAME,
                ExistingWorkPolicy.KEEP,
                request
            )
        }
    }

    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        Log.d(tag, "Starting progress sync...")

        return@withContext try {
            val unsyncedRows = db.progressDao().getUnsynced()

            if (unsyncedRows.isEmpty()) {
                Log.d(tag, "Nothing to sync — all progress already uploaded.")
                return@withContext Result.success()
            }

            Log.d(tag, "Uploading ${unsyncedRows.size} progress records...")

            val chunks = unsyncedRows.chunked(50)
            var successCount = 0
            var failCount = 0

            chunks.forEach { chunk ->
                try {
                    val rows = chunk.map { entity ->
                        ProgressRow(
                            id = entity.id,
                            student_id = entity.studentId,
                            lesson_id = entity.lessonId,
                            status = entity.status.toString(),
                            quiz_score = entity.quizScore,
                            quiz_total = entity.quizTotal,
                            // Convert Long to String for Supabase
                            completed_at = entity.completedAt?.let { java.time.Instant.ofEpochMilli(it).toString() },
                            synced_at = java.time.Instant.now().toString()
                        )
                    }

                    SupabaseClient.client
                        .from("student_progress")
                        .upsert(rows) {
                            onConflict = "id"
                        }

                    val chunkIds = chunk.map { it.id }
                    db.progressDao().markSyncedBatch(chunkIds)

                    successCount += chunk.size
                    Log.d(tag, "✓ Chunk uploaded: ${chunk.size} records")

                } catch (e: Exception) {
                    failCount += chunk.size
                    Log.e(tag, "✗ Chunk upload failed: ${e.message}")
                }
            }

            if (failCount > 0 && successCount == 0) {
                Result.retry()
            } else {
                Result.success()
            }

        } catch (e: Exception) {
            Log.e(tag, "Sync failed entirely: ${e.message}")
            Result.retry()
        }
    }
}

// ─────────────────────────────────────────────────────────────────────────────
// SyncPronunciationWorker
// Uploads all unsynced PRONUNCIATION_ATTEMPT rows to Supabase.
// ─────────────────────────────────────────────────────────────────────────────
class SyncPronunciationWorker(
    context: Context,
    params: WorkerParameters
) : CoroutineWorker(context, params) {

    private val tag = "SyncPronunciationWorker"
    private val db = AppDatabase.getInstance(context, DatabaseSeeder(context))

    companion object {
        const val WORK_NAME = "sync_pronunciation"

        @Serializable
        data class PronunciationRow(
            val id: String,
            val student_id: String,
            val lesson_id: String,
            val attempt_number: Int,
            val score: Int,
            val is_best: Boolean,
            val transcript: String,
            val feedback_json: String,
            val attempted_at: String, // Converted to String for Supabase
            val synced_at: String
        )

        fun enqueue(context: Context) {
            val constraints = Constraints.Builder()
                .setRequiredNetworkType(NetworkType.CONNECTED)
                .build()

            val jitterSeconds = Random.nextLong(5L, 35L)

            val request = OneTimeWorkRequestBuilder<SyncPronunciationWorker>()
                .setConstraints(constraints)
                .setInitialDelay(jitterSeconds, TimeUnit.SECONDS)
                .setBackoffCriteria(
                    BackoffPolicy.EXPONENTIAL,
                    WorkRequest.MIN_BACKOFF_MILLIS,
                    TimeUnit.MILLISECONDS
                )
                .addTag(WORK_NAME)
                .build()

            WorkManager.getInstance(context).enqueueUniqueWork(
                WORK_NAME,
                ExistingWorkPolicy.KEEP,
                request
            )
        }
    }

    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        Log.d(tag, "Starting pronunciation sync...")

        return@withContext try {
            val unsyncedAttempts = db.pronunciationDao().getUnsynced()

            if (unsyncedAttempts.isEmpty()) {
                return@withContext Result.success()
            }

            val chunks = unsyncedAttempts.chunked(50)

            chunks.forEach { chunk ->
                try {
                    val rows = chunk.map { entity ->
                        PronunciationRow(
                            id = entity.id,
                            student_id = entity.studentId,
                            lesson_id = entity.lessonId,
                            attempt_number = entity.attemptNumber,
                            score = entity.score,
                            is_best = entity.isBest,
                            transcript = entity.transcript,
                            feedback_json = entity.feedbackJson,
                            // Convert Long to String for Supabase
                            attempted_at = java.time.Instant.ofEpochMilli(entity.attemptedAt).toString(),
                            synced_at = java.time.Instant.now().toString()
                        )
                    }

                    SupabaseClient.client
                        .from("pronunciation_attempt")
                        .upsert(rows) {
                            onConflict = "id"
                        }

                    val chunkIds = chunk.map { it.id }
                    db.pronunciationDao().markSyncedBatch(chunkIds)

                } catch (e: Exception) {
                    Log.e(tag, "✗ Pronunciation chunk failed: ${e.message}")
                }
            }
            Result.success()

        } catch (e: Exception) {
            Result.retry()
        }
    }
}

// ─────────────────────────────────────────────────────────────────────────────
// SyncStudentWorker
// Pulls NEW students added by teacher from Supabase down to Room.
// ─────────────────────────────────────────────────────────────────────────────
class SyncStudentWorker(
    context: Context,
    params: WorkerParameters
) : CoroutineWorker(context, params) {

    private val tag = "SyncStudentWorker"
    private val db = AppDatabase.getInstance(context, DatabaseSeeder(context))

    companion object {
        const val WORK_NAME = "sync_students"

        fun enqueue(context: Context) {
            val constraints = Constraints.Builder()
                .setRequiredNetworkType(NetworkType.CONNECTED)
                .build()

            val jitterSeconds = Random.nextLong(0L, 15L)

            val request = OneTimeWorkRequestBuilder<SyncStudentWorker>()
                .setConstraints(constraints)
                .setInitialDelay(jitterSeconds, TimeUnit.SECONDS)
                .setBackoffCriteria(
                    BackoffPolicy.EXPONENTIAL,
                    WorkRequest.MIN_BACKOFF_MILLIS,
                    TimeUnit.MILLISECONDS
                )
                .addTag(WORK_NAME)
                .build()

            WorkManager.getInstance(context).enqueueUniqueWork(
                WORK_NAME,
                ExistingWorkPolicy.REPLACE,
                request
            )
        }
    }

    @Serializable
    data class StudentRow(
        val id: String,
        val class_id: String? = null,
        val full_name: String,
        val section: String,
        val grade_level: Int,
        val last_active: String? = null, // Supabase sends dates as Strings
        val created_at: String? = null
    )

    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        Log.d(tag, "Syncing student roster from Supabase...")

        return@withContext try {
            val remoteStudents = SupabaseClient.client
                .from("student")
                .select()
                .decodeList<StudentRow>()

            remoteStudents.forEach { row ->
                val entity = com.basahero.elearning.data.local.entity.StudentEntity(
                    id = row.id,
                    classId = row.class_id ?: "",
                    fullName = row.full_name,
                    section = row.section,
                    gradeLevel = row.grade_level,
                    // Safely parse the Supabase String back into an Android Long millisecond
                    lastActive = row.last_active?.let { java.time.Instant.parse(it).toEpochMilli() },
                    synced = true,
                    createdAt = row.created_at?.let { java.time.Instant.parse(it).toEpochMilli() } ?: System.currentTimeMillis()
                )
                db.studentDao().insertOrUpdate(entity)
            }

            Log.d(tag, "✓ Student roster sync complete")
            Result.success()

        } catch (e: Exception) {
            Log.e(tag, "Student sync failed: ${e.message}")
            Result.retry()
        }
    }
}